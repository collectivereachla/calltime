...
import { submitInvoice } from "./invoice-actions";
...
export function InvoicesView({ canManage, personId, myContract, paymentMethods, w9Threshold, w9OnFile, invoices }: Props) {
  ...
  return (
    <div className="space-y-8">
      {/* Submit panel — only relevant to the viewer's own invoice */}
      {!myContract ? (
        ...
        <div className="bg-card border border-bone rounded-card p-5">
          ...
          {noAmount ? (
            <p className="text-body-sm text-brick">
              Your contract doesn&apos;t list a payable dollar amount, so no invoice can be generated.
            </p>
          ) : w9Blocked ? (
            <div className="bg-tentative/10 border border-tentative/30 rounded-card px-4 py-3">
              <p className="text-body-sm text-ink">
                Payments of {money(w9Threshold)} or more need a W-9 on file before you can submit.
              </p>
            ...
            <>
              ...
              {needsDetails && (
                <div className="mb-3">
                  <label className="text-body-xs text-muted block mb-1">
                    {selectedMethod?.label || "Payment"} details (where should it go?)
                  </label>
                  <input
                    value={details}
                    onChange={(e) => setDetails(e.target.value)}
                    placeholder={selectedMethod?.method === "cashapp" ? "$YourCashtag" : "Account / handle"}
                    className="w-full px-3 py-2 text-body-sm rounded border border-bone bg-paper text-ink focus:outline-none focus:border-brick"
                  />
                </div>
              )}
              ...
              {w9Required && w9OnFile && (
                <p className="text-body-xs text-confirmed mb-3">W-9 on file ✓ (required at {money(w9Threshold)}+)</p>
              )}
              ...
              <button
                ...
                disabled={submitting || paymentMethods.length === 0 || !method}
              ...
              <p className="text-body-xs text-muted mt-2">
                The amount is locked to your signed contract. Any approved stipend is added by your manager as a separate line.
              </p>
            </>
          )}
        </div>
      ...
      {invoices.length > 0 && (
        <div>
          <p className="text-body-xs text-muted uppercase tracking-wider mb-2">
            {canManage ? "All invoices" : "Your invoices"}
          </p>
          <div className="space-y-2">
            {invoices.map((inv) => (
              <div key={inv.id} className="bg-card border border-bone rounded-card px-4 py-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      {canManage && <span className="text-body-sm font-medium text-ink">{inv.person_name}</span>}
                      <span className={`px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider ${STATUS_STYLE[inv.status] || "bg-ash/10 text-ash"}`}>
                        {inv.status}
                      </span>
                      {inv.w9_required && (
                        <span className="px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider bg-brick/10 text-brick">W-9</span>
                      )}
                    </div>
                ...
                {inv.lines.length > 1 && (
                  <div className="mt-2 pt-2 border-t border-bone/60 space-y-0.5">
                    {inv.lines.map((l, i) => (
                      <div key={i} className="flex items-center justify-between text-body-xs text-ash">
